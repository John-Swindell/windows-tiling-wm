...
if [[ "$keep_apps" == "true" ]]; then
  log "Keeping Zebar app install because --keep-apps was passed"
  exit 0
fi

if [[ "$all_versions" == "true" ]]; then
  parent="$WINWM_APP_ROOT_WSL/zebar"
  if [[ -d "$parent" ]]; then
  ...
  remove_managed_dir "$ZEBAR_INSTALL_ROOT_WSL" "$force"
  manifest_remove_path "$ZEBAR_INSTALL_ROOT_WIN"
  rmdir "$WINWM_APP_ROOT_WSL/zebar" 2>/dev/null || true
fi
